...
public interface FunctionUtils {
    ...
    public static void main(String[] args) {
        ...
        // 模拟 数据
        System.out.println("开始累加数据");
        for (int i = 0; i < 100; i++) {
            list.add(new User(SnowFlakeIdGenerate.generateId(), "name"+String.valueOf(SnowFlakeIdGenerate.generateId())));
        }
        System.out.println("累加数据结束");

        long start = System.currentTimeMillis();


        Map<String, Long> longMap = toMap(list, User::getName, User::getId);

        longMap.forEach((k,v)->{
            System.out.println(k + "-----" + v);
        });

        /*List<String> parallel = batchAndParallel(list, vos -> {
            return vos.get(0);
        }, user -> {
            return user.getName();
        }, 6);
